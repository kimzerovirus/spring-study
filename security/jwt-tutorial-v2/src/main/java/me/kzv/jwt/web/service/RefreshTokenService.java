package me.kzv.jwt.web.service;
...
import me.kzv.jwt.web.repository.RefreshTokenRepository;
...
public class RefreshTokenService {
    ...
    public JwtResponseDto refreshToken(JwtRefreshRequestDto refreshRequestDto) {
        ...
        if (tokenOpt.isEmpty()) {
            throw new RuntimeException(String.format("Refresh Token %s not found!", refreshRequestDto.getRefreshToken()));
        }
        var token = tokenOpt.get();
        if (isTokenExpired(token.getExpiration())) {
            refreshTokenRepository.delete(token);
            throw new RuntimeException(String.format("Refresh Token %s was expired!", refreshRequestDto.getRefreshToken()));
        }
